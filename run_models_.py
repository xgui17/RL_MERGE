...
if regression_flag == 0:
    ...
    #np.save(os.path.join(save_dir, 'model_' + date_and_time + '_test_metrics'  + '.npy'), test_metrics)
        ...
        # Save train_metrics
    np.savez(os.path.join(save_dir, f"model_{date_and_time}_train_metrics.npz"), 
            train_gene_ID=train_metrics[0], 
            train_pred=train_metrics[1], 
            train_labels=train_metrics[2], 
            train_pearson_vec=train_metrics[3], 
            train_loss_vec=train_metrics[4])

    # Save valid_metrics
    np.savez(os.path.join(save_dir, f"model_{date_and_time}_valid_metrics.npz"), 
            valid_gene_ID=valid_metrics[0], 
            valid_pred=valid_metrics[1], 
            valid_labels=valid_metrics[2], 
            valid_pearson_vec=valid_metrics[3], 
            valid_loss_vec=valid_metrics[4])

    # Save test_metrics
    np.savez(os.path.join(save_dir, f"model_{date_and_time}_test_metrics.npz"), 
            test_gene_ID=test_metrics[0], 
            test_pred=test_metrics[1], 
            test_labels=test_metrics[2], 
            test_pearson_vec=test_metrics[3], 
            test_loss_vec=test_metrics[4])
